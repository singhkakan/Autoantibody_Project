...
nitr=75 # number of iterations (going upto 95, 46th iteration was found to be optimum)
...
} #### the lowest sad or "sum of deviations" occurs at the 74th iteration.
#qcut at this i=46 is 0.0074
...
###### B8 vs N8
Few autoantibodies are differentially expressed at 8 weeks of age
```{r 8 weeks comparison, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
jWRS=function(j) wilcox.test(W[j,1:3],W[j,7:9])$p.value
myWRS=mapply(jWRS,1:70)
myQ=p.adjust(myWRS,method="BH")
pCDFlist=list(2*((1:151)/252))
for(j in 2:70) pCDFlist=c(pCDFlist,list(pCDFlist[[1]]))
myQ.disc=p.discrete.adjust(myWRS, pCDFlist, method = "BHmidp")
# order the features by statistical significance
odx=order(myWRS)
# compute HC scores
...
colnames(out)=c("p-value","q-value (BH)","q-values (BH midP)","HC score","Tri-Mean rank-it")
out=out[odx,]
DE.IgGj=which(myWRS<0.025)
# make table
tblGj=round(out[hcDX,],4)
head(tblGj)
```
...
##### B28 vs N24
```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
kWRS=function(k) wilcox.test(W[k,4:6],W[k,10:12])$p.value
myWRS=mapply(kWRS,1:70)
myQ=p.adjust(myWRS,method="holm")
pCDFlist=list(2*((1:151)/252))
for(k in 2:70) pCDFlist=c(pCDFlist,list(pCDFlist[[1]]))
myQ.disc=p.discrete.adjust(myWRS, pCDFlist, method = "none")
# order the features by statistical significance
odx=order(myWRS)
# compute HC scores
hcG=hc.score(myWRS)
...
colnames(out)=c("p-value","q-value (BH)","q-values (BH midP)","HC score","Tri-Mean rank-it")
out=out[odx,]
DE.IgGk=which(myWRS<0.1)
# make table

tblGk=round(out[hcDX,],4)
head(tblGk)
