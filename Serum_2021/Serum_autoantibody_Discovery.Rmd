...
#github token
#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
#Expiry 07/07/24
```
...
IgG_SNR <- read.csv("../../3_Parkinsons_disease/Autoantibody_Data/SerumAntibodies_2021/IgG_MCF_SNR.csv",  header=T, row.names = 1)[,1:15]
...
for(i in 1:nitr){
   ...
   X=IgG_raw$X_norm_1
   iWRS=function(i) t.test(X[i,1:6],X[i,7:15])$p.value
   qMat=matrix(rep(mapply(iWRS,1:73),15),ncol=15)
...
abline(0,1)
topTable(fit.cont,number=30,adjust="BH", coef=1)
...
mydata <- as.matrix(IgG_raw$W_1)[1:73,]
hits <- rownames(fitObjres)
Y=matrix(nrow=30,ncol=12)
for (i in 1:30) {
   Y[i,] <- mydata[hits[i],]
}
rownames(Y) <- hits
...
hits <- colnames(Y)[1:30] #change only colnames so figure headings are meaningful
...
colnames(Y)[18] <- "Mito Antigen"
colnames(Y)[23] <- "PM/Scl100"
colnames(Y)[25] <- "RPP P2"
...
for (i in 1:30){
   ...
   p<- ggplot(Y, aes(x=Strain, y=Y[,i])) +
     ...
     geom_jitter(aes(fill=AGE), pch=21,size=2,  position=position_jitterdodge(0.40)) +
      scale_color_jco() +
     scale_fill_aaas() +
      guides(color="none") +
      theme_minimal() +
...
par(mfrow=c(2,5))
for (i in 1:26){
   filename <- paste(hits[i], "Serum.IgG.tiff", sep="")
   p<- ggplot(Y, aes(x=Strain, y=Y[,i])) +
      geom_boxplot(aes(color=Strain), outlier.shape = NA, width = 0.4, coef=1, varwidth=F, show.legend = F, size=0.5) +
     geom_jitter(aes(fill=AGE), size=2, pch=21, width = 0.25) +
      scale_color_jco() +
     scale_fill_aaas() +
      theme_minimal() +
      chart_design +
      ...
      labs(title=colnames(Y[i]), hjust=0.5) +
      scale_x_discrete("Strain", labels=c("BALB/c", "NOD")) + 
     ylim(5.5,27.5) + guides(color="none")
   tiff(filename, units="in", width=3.2, height=3.25, res=300)
   print(p)
   dev.off()
}
```
