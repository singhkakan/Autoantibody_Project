...
IgG_NSI <- read.csv("IgG_MCF_SSK_546_Tear_NSI.csv",  header=T, nrows=80)[1:16]#, row.names = 1)
IgG_SNR <- read.csv("IgG_MCF_SSK_546_Tear_SNR.csv",  header=T, nrows=80)[1:16]#, row.names = 1)
...
colData <- as.data.frame(cbind(c(colnames(IgG_NSI[,1:15])), Strain))
...
IgG_raw=list()
IgG_SNR$average <- apply(cbind(rowMeans(as.matrix(IgG_SNR)[,1:9]), rowMeans(as.matrix(IgG_SNR)[,10:15])), 1, max)
IgG_SNR$med <- apply(cbind(rowMedians(as.matrix(IgG_SNR)[,1:9]), rowMedians(as.matrix(IgG_SNR)[,10:15])), 1, max)

IgG_raw$NSI <- as.matrix(IgG_NSI[which(IgG_SNR$med>2),1:15])
IgG_raw$SNR <- as.matrix(IgG_SNR[which(IgG_SNR$med>2),1:15])[,1:15]
...
```{r}
#Removing male BALB/c 6
library(limma)
dataN <- log2(IgG_raw$NSI[,-15] + 0.5) 
...
conditions<- paste(colData$Strain[-c(15)],sep=".")
conditions <- factor(conditions, levels=unique(conditions))
design <- model.matrix(~0+ conditions)
colnames(design) <- levels(conditions)

v <- voom(counts=(IgG_raw$NSI[,-c(15)]), design, plot=TRUE, normalize="quantile")
...
print(NTvnTvBT[which(NTvnTvBT$logFC.NOD>0.5),c(1:2,5,8,12)])
```
...
dae <- DGEList(counts=(IgG_raw$NSI+0.5))
...
Y_combined <- Y[,c(NTvnTvBT$Antigen[which(NTvnTvBT$adj.P.Val.NOD<0.1 & NTvnTvBT$adj.P.Val.NOR < 0.1 & NTvnTvBT$logFC.NOD>0)])]
