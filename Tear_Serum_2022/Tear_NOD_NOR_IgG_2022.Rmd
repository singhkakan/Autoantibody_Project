...
IgG_raw=list()
IgG_SNR$average <- rowMeans(as.matrix(IgG_SNR)[,1:15])
IgG_SNR$med <- rowMedians(as.matrix(IgG_SNR))

IgG_raw$NSI <- as.matrix(IgG_NSI[which(IgG_SNR$med>1.8),])
IgG_raw$SNR <- as.matrix(IgG_SNR[which(IgG_SNR$med>1.8),])[,1:15]
...
v <- voom(counts=(IgG_raw$NSI*IgG_raw$SNR+0.5), design, plot=TRUE, normalize="quantile")
