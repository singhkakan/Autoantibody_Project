...
Y_combined <- Y[,c(fitObjDE$Autoantigen[which(fitObjDE$t.nTvBT>0 & fitObjDE$t.NTvBT>0 & fitObjDE$P.value.nTvBT < 0.01)])]
...
tiff("Serum_IgA_hits.tiff", units="in", width=5.5, height=4, res=300)
...
#####Filtering Data based on low Signal to Noise ratio
```{r Filtering rows with SNR<2.4}
IgG_raw=list()
IgG_SNR$average <- rowMeans(IgG_SNR)
IgG_SNR$med <- rowMedians(as.matrix(IgG_SNR))

IgG_raw$NSI <- as.matrix(IgG_NSI[which(IgG_SNR$med>2.4),])
IgG_raw$SNR <- as.matrix(IgG_SNR[which(IgG_SNR$med>2.4),])[,1:15]
```
...
dataN <- log(IgG_raw$NSI * IgG_raw$SNR +0.5) 
countData = as.data.frame(dataN)
boxplot(as.data.frame((dataN)),main="Log2 NSI*SNR")
...
NTvsBT <- topTable(fit.cont, coef=1, number=45, adjust.method = 'BH')
nTvsBT <- topTable(fit.cont, coef=2, number=45, adjust.method = 'BH')
NTvsBT$Antigen <- row.names(NTvsBT)
nTvsBT$Antigen <- row.names(nTvsBT)
NTvnTvBT <- full_join(NTvsBT, nTvsBT, by="Antigen")
rownames(NTvnTvBT) <- NTvnTvBT$Antigen
...
```{r}
plotMA((IgG_raw$NSI * IgG_raw$SNR + 1))
plotMDS(fit)
plotMA(fit)
plotMA(fit.cont, coef=1)
plotMA(fit.cont, coef=2)
...
v <- voom(counts=(IgG_raw$NSI*IgG_raw$SNR+1), design, plot=TRUE, normalize="scale")
...
#####Density plots post voom normalization ####
```{r Density plots}
dae <- DGEList(counts=(IgG_raw$NSI * IgG_raw$SNR + 1))
keep <- filterByExpr(dae, design)
dae <- dae[keep,,keep.lib.sizes=FALSE]
dae <- calcNormFactors(dae)
logCPM <- cpm(dae, log=TRUE, prior.count=0)

#df_dseq = melt(dae$counts, variable.name = "Samples", value.name = "count")# reshape the matrix 
df_dseq = melt(log(dae$counts), variable.name = "Samples", value.name = "count")# reshape the matrix 
df_dseq$Strain <- factor(substr(df_dseq$Var2, 1,3))
mycolors <- colorRampPalette(brewer.pal(8,"Set1"))(15)
...
#####DE Analysis using voom normalized counts
```{r}
results<-decideTests(fit.cont,adjust.method="fdr", p=0.1)
summary(results)
...
fitObjDE <-fitObj[DElist,]
write.csv(fitObjDE,file="IgG_Serum_Voom_Hits.csv", sep=',')
fitObjDE$Autoantigen <- rownames(fitObjDE)
```
...
chart_design <- theme(
   ...
   strip.background = element_blank(), 
   strip.placement = "outside")
mydata <- as.matrix(log2(IgG_raw$NSI*IgG_raw$SNR +1))
hits <- rownames(NTvnTvBT)
Y=matrix(nrow=37,ncol=15)
for (i in 1:37) {
   Y[i,] <- mydata[hits[i],]
}
rownames(Y) <- hits
