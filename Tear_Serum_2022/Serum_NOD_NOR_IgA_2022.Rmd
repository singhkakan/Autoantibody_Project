...
IgA_raw$NSI <- as.matrix(IgA_NSI[which(IgA_SNR$med>2 & IgA_NSI$med>0),])[,1:15]
IgA_raw$SNR <- as.matrix(IgA_SNR[which(IgA_SNR$med>2 & IgA_NSI$med>0),])[,1:15]
```

#### Visualizing Filtered Data ####
```{r}
dataN <- log2(IgA_NSI[,1:15] * IgA_SNR[,1:15] + 0.5) 
...
conditions<- paste(colData$Strain[-c(1,3)],sep=".")
conditions <- factor(conditions, levels=unique(conditions))
design <- model.matrix(~0+ conditions)
colnames(design) <- levels(conditions)

v <- voom(counts=(IgA_raw$NSI*IgA_raw$SNR + 0.5)[,-c(1,3)], design, plot=TRUE, normalize="quantile")
...
tiff("ColSum_Serum_IgA_vs_IgG.tiff", units="in", width=5.5, height=4, res=300)
setwd("~/Documents/3_Parkinsons_disease/Autoantibody_Data/Tear_Auto_Validation_2022/")
