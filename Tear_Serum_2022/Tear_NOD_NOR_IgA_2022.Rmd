...
chart_design <- theme(
   #plot.title = element_text(color = "Black", size = 16, face = "bold", margin = margin(b=15), hjust=0.4),,
   axis.text.x = element_text(size=15, margin = margin(b=5)),
   ...
   strip.text.x = element_text(size =18, margin = margin(b=15), face='bold', hjust=0.4),
   strip.background = element_blank(), 
   strip.placement = "outside")
...
setwd("~/Documents/3_Parkinsons_disease/Autoantibody_Data/Tear_Auto_Validation_2022/IgA_data/")
tiff("Tear_IgA_hits.tiff", units="in", width=10.5, height=9, res=300)
...
```{r}
setwd('~/Documents/3_Parkinsons_disease/Autoantibody_Data/Tear_Auto_Validation_2022/')
Tear_IgAhits <- read.csv("NTvnTvBT_Tear_IgA.csv", header=T)
Tear_IgGhits <- read.csv("NTvnTvBT_Tear_IgG.csv", header=T)
