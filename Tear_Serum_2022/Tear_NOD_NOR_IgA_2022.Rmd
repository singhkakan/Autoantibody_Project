...
write.csv(NTvnTvBT, "NTvnTvBT_Tear_.csv")
...
chart_design <- theme(
   ...
   strip.text.x = element_text(size =17, margin = margin(b=20), face='bold', hjust=0.4),
   strip.background = element_blank(), 
   strip.placement = "outside")


Y_combined <- Y[,c(NTvnTvBT$Antigen[which(NTvnTvBT$P.Value.x<0.05 & NTvnTvBT$P.Value.y < 0.05 & NTvnTvBT$logFC.x>0)])]
Y_combined[,14:15] <- Y[,c("IF", "La SSB")]
Y_combined[,16:17] <- Y[,46:47]
Y_combined <- gather(Y_combined, "Antigen", "V Counts", 1:15)

tiff("Tear_IgA_hits.tiff", units="in", width=10, height=8, res=300)
