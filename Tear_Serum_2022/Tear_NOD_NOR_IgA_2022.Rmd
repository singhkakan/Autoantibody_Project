...
NTvsBT <- topTable(fit.cont, coef=1, p.value=1, number=35, adjust.method = 'BH')
nTvsBT <- topTable(fit.cont, coef=2, p.value=1, number=35, adjust.method = 'BH')
...
fitObjDE$Autoantigen[which(fitObjDE$t.NTvBT>0 & fitObjDE$P.value.nTvBT<0.01)]
Y_combined <- Y[,c(fitObjDE$Autoantigen[which(fitObjDE$t.NTvBT>0 & fitObjDE$P.value.nTvBT<0.025 & fitObjDE$P.value.NTvBT<0.025)])]
Y_combined[,10:11] <- Y[,44:45]
#Y_combined <- Y_combined[,-c(4,12)] #Removed "GP210", "NXP2"
Y_combined <- gather(Y_combined, "Antigen", "V Counts", 1:9)

tiff("Tear_IgA_hits.tiff", units="in", width=8, height=8.5, res=300)
ggplot(Y_combined, aes(x=Strain, y=`V Counts`, fill=Strain)) +
      geom_boxplot(outlier.shape = NA, width = 0.6, coef=1, varwidth=F, show.legend = T, size=0.8, position = position_dodge(0.9)) +
      geom_jitter(color = "darkgray", alpha =0.6, size=2.75, show.legend = F, position = position_jitterdodge(dodge.width=0.9))+
